...
def get_best_bin_dir() -> str:
    for d in [SKILLS_NATIVE_BIN_DIR, WORKERS_BIN_DIR, ACTUATORS_BIN_DIR]:
        if os.path.exists(d):
            return d
    return SKILLS_NATIVE_BIN_DIR
...
class ActuatorDispatcher:
    ...
    async def dispatch_tool(self, tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # ─── OMARCHY AGENTIC OS SKILLS (FAST C++ & PYTHON) ───────────────────
        elif tool in ["omarchy_control", "omarchy_action", "omarchy"]:
            domain = str(args.get("domain", "hyprland")).lower().strip()
            action = str(args.get("action", "")).lower().strip()
            target = str(args.get("target", "")).strip()

            try:
                from omarchy_skills import execute_omarchy_action
                res = await execute_omarchy_action(domain, action, target)
                return res
            except Exception as e:
                # Direct C++ worker fallback
                cpp_args = [domain, action]
                if target:
                    cpp_args.append(target)
                cpp_res = await self.execute_cpp_worker("omarchy_ctrl", cpp_args)
                if cpp_res.get("success"):
                    return cpp_res
                return {"success": False, "domain": domain, "action": action, "target": target, "error": str(e)}
        ...
        # ─── SYSTEMD SERVICES & SYSTEM LOGS ──────────────────────────────────
            ...
            if action == "list":
                ...
                unit_map = {
                    ...
                    "jarvis.service": "jarvis-gateway.service",
                    "friday": "jarvis-gateway.service",
                    "friday.service": "jarvis-gateway.service",
                }
                resolved_unit = unit_map.get(unit_clean, unit)
                is_user_unit = resolved_unit in ["hermes-gateway.service", "openclaw-gateway.service", "jarvis-gateway.service", "friday-gateway.service"]
                if is_user_unit:
    ...
    async def _handle_forge_tool(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            vault_base = Path(os.getcwd()) / "memory" / "vault"
            if not vault_base.exists():
                vault_base = Path(os.getcwd()) / "jarvis-memory"
            obsidian_skill_dir = vault_base / "skills" / tool_name
...
"""
    ...
    async def _handle_delete_custom_tool(self, tool_name: str) -> Dict[str, Any]:
        ...
        for vault_base in [Path(os.getcwd()) / "memory" / "vault", Path(os.getcwd()) / "jarvis-memory"]:
            obsidian_skill_dir = vault_base / "skills" / tool_name
    ...
    async def _run_cbm_cli(self, args: List[str], prefix: Optional[List[str]] = None) -> Dict[str, Any]:
        cbm_bin = shutil.which("codebase-memory-mcp") or os.path.expanduser("~/.local/bin/codebase-memory-mcp")
    ...
    async def _handle_codebase_tool(self, tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not os.path.exists(cbm_bin):
            return {"success": False, "error": f"codebase-memory-mcp binary not found at {cbm_bin}."}
