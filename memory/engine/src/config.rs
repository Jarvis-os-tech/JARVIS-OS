...
impl Default for Config {
    fn default() -> Self {
        ...
        // Priority 1: Explicit env var (JARVIS_MEMORY_PATH or FRIDAY_MEMORY_PATH)
        // Priority 2: Local ./memory/vault, ./jarvis-memory or ./friday-memory (or ./JARVIS-MEMORY) in current working directory
        // Priority 3: ~/.jarvis/memory or ~/.friday/memory
        let base_dir = if let Ok(custom) = std::env::var("JARVIS_MEMORY_PATH").or_else(|_| std::env::var("FRIDAY_MEMORY_PATH")) {
            PathBuf::from(custom)
        } else if std::path::Path::new("memory/vault").exists() {
            std::fs::canonicalize("memory/vault").unwrap_or_else(|_| PathBuf::from("memory/vault"))
        ...
        let db_path = if let Ok(custom_db) = std::env::var("JARVIS_MEMORY_DB").or_else(|_| std::env::var("FRIDAY_MEMORY_DB")) {
        ...
        let http_port = std::env::var("JARVIS_MEMORY_PORT")
            .or_else(|_| std::env::var("FRIDAY_MEMORY_PORT"))
            .ok()
