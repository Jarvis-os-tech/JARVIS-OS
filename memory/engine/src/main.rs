...
fn initialize_single_db(config: &Config) -> Result<serde_json::Value, Box<dyn std::error::Error>> {
    ...
    if active.is_empty() {
        ...
        let init_fact = MemoryNode {
            ...
            content: "J.A.R.V.I.S. Universal Memory Core initialized with 11 WAL tables and FTS5 triggers.".to_string(),
...
fn execute_init(config: &Config, explicit_path: Option<PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(path) = explicit_path {
        ...
        let home_db = dirs::home_dir().unwrap_or_else(|| PathBuf::from(".")).join(".jarvis").join("memory").join("memory.db");
